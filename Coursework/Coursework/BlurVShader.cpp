...
BlurVShader::BlurVShader(ID3D11Device* device, HWND hwnd, int w, int h) : BaseShader(device, hwnd)
{
	sWidth = w;
	sHeight = h;
	initShader(L"blurV_cs.cso", NULL);
}
...
void BlurVShader::initShader(const wchar_t* cfile, const wchar_t* blank)
{
...
void BlurVShader::createOutputUAV()
{
...
void BlurVShader::setShaderParameters(ID3D11DeviceContext* dc, int size, float deviation, ID3D11ShaderResourceView* texture0)
{
...
void BlurVShader::unbind(ID3D11DeviceContext* dc)
{
